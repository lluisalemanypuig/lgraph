...
void xd_metric_clustering_gcc(const xxgraph *G) {
	double c = networks::metrics::clustering::gcc(G);
	cout << floatpointout_metric(c) << endl;
}

void xd_metric_clustering_mlcc(const xxgraph *G) {
	double c = networks::metrics::clustering::mlcc(G);
	cout << floatpointout_metric(c) << endl;
}

err_type xd_metric_clustering(const string& c, ifstream& fin)
{
	...
	fin >> field;
	if (field != "INPUT") {
		cerr << ERROR("xd_metric_clustering.cpp", "xd_metric_clustering") << endl;
	...
	if (c == "gcc") {
		xd_metric_clustering_gcc(G);
	}
	else if (c == "mlcc") {
		xd_metric_clustering_mlcc(G);
	}
	else {
		cerr << ERROR("xd_metric_clustering.cpp", "xd_metric_clustering") << endl;
