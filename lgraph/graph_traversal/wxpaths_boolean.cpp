...
(const wxgraph<T> *G, node source, node target, boolean_path<T>& p) {
	node_path<T> path;
...
(const wxgraph<T> *G, node source, node target, boolean_path_set<T>& ps) {
...
// one path
template<class T> void wxpath
(const wxgraph<T> *G, node source, std::vector<boolean_path<T> >& ps) {
	std::vector<node_path<T> > nps;
	wxpath(G, source, nps);
...
(const wxgraph<T> *G, node source, std::vector<boolean_path_set<T> >& ps)
{
	...
	std::vector<node_path_set<T> > node_ps;
	wxpaths(G, source, node_ps);

	ps = std::vector<boolean_path_set<T> >(N);
...
// one path
template<class T> void wxpath
(const wxgraph<T> *G, std::vector<std::vector<boolean_path<T> > >& all_all_paths)
{
	const size_t N = G->n_nodes();
	...
	all_all_paths = std::vector<boolean_path_set<T> >(N);
	for (size_t i = 0; i < N; ++i) {
...
// all paths
template<class T> void wxpaths
(const wxgraph<T> *G, std::vector<std::vector<boolean_path_set<T> > >& all_all_paths)
{
	const size_t N = G->n_nodes();

	std::vector<std::vector<node_path_set<T> > > all_all_node_paths;
	wxpaths(G, all_all_node_paths);

	all_all_paths =
		std::vector<std::vector<boolean_path_set<T> > >
		(N, std::vector<boolean_path_set<T> >(N));
